export interface CharacterVariation {
  ...
  referenceImageUrl?: string; // 角色变体参考图的原始 URL（API 返回的 CDN 地址），用于支持仅接受 URL 的 API（如 Seedream）
...
export interface Character {
  ...
  referenceImageUrl?: string; // 角色参考图的原始 URL（API 返回的 CDN 地址），用于支持仅接受 URL 的 API（如 Seedream）
...
export interface Scene {
  ...
  referenceImageUrl?: string; // 场景参考图的原始 URL（API 返回的 CDN 地址），用于支持仅接受 URL 的 API（如 Seedream）
  status?: 'pending' | 'generating' | 'completed' | 'failed'; // 生成状态，用于loading状态持久化
}
...
export interface Prop {
  ...
  referenceImageUrl?: string; // 道具参考图的原始 URL（API 返回的 CDN 地址），用于支持仅接受 URL 的 API（如 Seedream）
  status?: 'pending' | 'generating' | 'completed' | 'failed'; // 生成状态，用于loading状态持久化
}
...
/**
 * 剧集/集数 —— 由一章或多章小说生成的剧本
 * 一个 NovelEpisode 对应一集剧本
 */
export interface NovelEpisode {
  id: string;
  name: string;                 // 集名，如"第1集"
  chapterIds: string[];         // 关联的章节 ID 列表
  chapterRange: string;         // 可读的章节范围描述，如"第1-3章"
  script: string;               // AI 生成的剧本文本
  status: 'pending' | 'generating' | 'completed' | 'failed';
  createdAt: number;
  updatedAt: number;
}
