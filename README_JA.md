...
### Phase 01: 小説と脚本 (Novel & Script)
*   **プロジェクト設定の統合**: プロジェクトのタイトル、ジャンル、ビジュアルスタイル、言語などの設定を小説管理ページで直接管理できます。ページ切り替え不要です。
*   **小説 → 脚本**: `.txt` 形式の小説ファイルをアップロードし、チャプターを自動解析（ページネーション対応）。チャプターを選択してエピソードを作成し、AI が脚本に変換します。
*   **エピソード分離**: 各エピソードはキャラクター、ストーリーボード、アセットのデータを独立して管理します。
...
*   **手動編集**: キャラクターの視覚的説明、ショットプロンプト、キャラクターリスト、アクション説明、セリフを編集可能です。
...
### アカウント管理
*   サイドバーまたはプロジェクト一覧のユーザー名をクリックして「アカウント設定」を開きます。
*   現在のパスワードを確認した上で、ユーザー名とパスワードを変更できます。

## 技術スタック

*   **Frontend**: React 19, Vite, Tailwind CSS
*   **Backend**: Express.js, MySQL, JWT 認証
*   **AI**: マルチプロバイダー テキスト/画像/ビデオ API、統一アダプターレイヤー（`services/adapters`、`types/model.ts` 参照）
*   **Storage**: MySQL でプロジェクト、アセット、モデル設定、ユーザー設定を永続化。`user_id` でデータ分離
*   **Files**: 小説等のアップロードファイルはサーバー側の `uploads/` に保存、ユーザーごとに分離
...
1.  **ログイン**: デフォルトアカウント（admin / admin123）でログインまたは新規ユーザー登録。ユーザー名をクリックしてアカウント情報を変更可能です。
2.  **モデル設定**: モデル設定画面でプロバイダーの API Key を入力し、使用するテキスト/画像/ビデオモデルを選択します。
3.  **プロジェクトと脚本**: Phase 01 の小説管理ページでプロジェクト設定（タイトル、ジャンル、スタイル）を管理。`.txt` 小説をアップロード → チャプター解析 → エピソード作成 → 脚本生成。ストーリーを直接貼り付けることも可能です。
4.  **アセット**: Phase 02 でキャラクターシートとシーンコンセプトを生成します。
5.  **ショットとエクスポート**: Phase 03 でまず開始フレームを生成し、必要に応じて終了フレームを追加または九分割プレビューを使用。Phase 04 でプレビューとエクスポート。
