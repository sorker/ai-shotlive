...
### データのエクスポートとインポート
*   **エクスポート**：システム設定から、現在のユーザーのすべてのデータベース（プロジェクト、アセット、ビジュアルスタイル、モデル設定、ユーザー設定）と `data/` フォルダ内のメディアファイルを ZIP アーカイブとしてエクスポートできます。
*   **インポート**：ZIP バックアップファイルをアップロードすると、システムが自動的に新しいユーザーアカウントを作成し、すべてのデータをインポートします。完了後に新しいユーザー名とデフォルトパスワードが表示されます。
*   サーバー間のデータ移行、定期バックアップ、災害復旧に最適です。
...
*   **Files**: 小説アップロードは `uploads/`、メディアファイル（画像/動画）は `data/` に保存、ユーザー/プロジェクトごとに分離
*   **Backup**: ZIP アーカイブによるエクスポート/インポート（データベース + メディアファイル）。インポート時に新規ユーザーを自動作成
