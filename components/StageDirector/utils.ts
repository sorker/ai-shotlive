...
export const getRefImagesForShot = (shot: Shot, scriptData: ProjectState['scriptData']): RefImagesResult => {
  ...
  // 1. 场景参考图（环境/氛围） - 优先级最高
  // 优先使用 URL（Seedream 等 API 需要 URL 格式），回退到 base64
  const scene = scriptData.scenes.find(s => String(s.id) === String(shot.sceneId));
  const sceneImg = scene?.referenceImageUrl || scene?.referenceImage;
  if (sceneImg) {
    referenceImages.push(sceneImg);
  }
  ...
  if (shot.characters) {
    shot.characters.forEach(charId => {
      ...
      if (varId) {
        ...
        const varImg = variation?.referenceImageUrl || variation?.referenceImage;
        if (varImg) {
          referenceImages.push(varImg);
          return; // 使用变体图片而不是基础图片
        }
      }

      // 基础参考图（优先 URL）
      const charImg = char.referenceImageUrl || char.referenceImage;
      if (charImg) {
...
export const buildKeyframePrompt = (
  ...
  // 针对起始帧和结束帧的特定指导
  let frameSpecificGuide: string;
  if (frameType === 'start') {
    frameSpecificGuide = `【起始帧要求】建立清晰的初始状态和场景氛围,人物/物体的起始位置、姿态和表情要明确,为后续运动预留视觉空间和动势。`;
  } else if (hasStartFrameRef) {
    frameSpecificGuide = `【结束帧要求 - 基于首帧参考】
⚠️ 参考图中的第一张图片是本镜头的首帧画面，请严格基于首帧进行延续:
• 场景延续: 保持与首帧完全相同的场景环境、光影基调和色彩风格
• 角色延续: 角色外观、服装必须与首帧完全一致，仅改变姿态、表情和位置
• 动作完成: 展现分镜动作执行完毕后的最终状态
• 视角变化: 体现镜头运动（${cameraMovement}）带来的构图和视角变化
• 情绪推进: 在首帧基础上体现情绪的变化和发展
⚠️ 首帧一致性是最高优先级！不要改变场景和角色的基本外观！`;
  } else {
    frameSpecificGuide = `【结束帧要求】展现动作完成后的最终状态,人物/物体的终点位置、姿态和情绪变化,体现镜头运动带来的视角变化。`;
  }
...
export const buildKeyframePromptWithAI = async (
  ...
  // 先构建基础提示词
  const basicPrompt = buildKeyframePrompt(basePrompt, visualStyle, cameraMovement, frameType, propsInfo, hasStartFrameRef);
...
export const extractBasePrompt = (fullPrompt: string, fallback: string): string => {
  if (!fullPrompt) return fallback;

  // 新格式：以 ━━━ 分隔线作为追加段落的起点
  const separatorIndex = fullPrompt.indexOf('\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  if (separatorIndex > 0) {
    return fullPrompt.substring(0, separatorIndex).trim();
  }
