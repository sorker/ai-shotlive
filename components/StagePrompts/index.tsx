...
const StagePrompts: React.FC<Props> = ({ project, updateProject }) => {
  ...
  const handleSaveEdit = () => {
    ...
    switch (editingPrompt.type) {
      case 'character':
        PS.patchCharacter(pid, editingPrompt.id, { visualPrompt: editingPrompt.value });
        break;
      case 'character-variation':
        if (editingPrompt.variationId) {
          PS.patchVariation(pid, editingPrompt.id, editingPrompt.variationId, { visualPrompt: editingPrompt.value });
        }
        break;
      case 'scene':
        PS.patchScene(pid, editingPrompt.id, { visualPrompt: editingPrompt.value });
        break;
      case 'prop':
        PS.patchProp(pid, editingPrompt.id, { visualPrompt: editingPrompt.value });
        break;
      case 'keyframe':
        if (editingPrompt.shotId) {
          PS.patchKeyframe(pid, editingPrompt.shotId, editingPrompt.id, { visualPrompt: editingPrompt.value });
        }
        break;
      case 'video':
        if (editingPrompt.shotId) {
          PS.patchVideoInterval(pid, editingPrompt.shotId, editingPrompt.id, { videoPrompt: editingPrompt.value });
        }
        break;
    }
